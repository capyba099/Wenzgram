...
QByteArray SessionDeviceBindingMaterialValue;
...
void SetSessionDeviceBindingMaterial(const QByteArray &material) {
	SessionDeviceBindingMaterialValue = material;
}

const QByteArray &SessionDeviceBindingMaterial() {
	return SessionDeviceBindingMaterialValue;
}
...
MTP::AuthKeyPtr CreateLocalKey(
	...
	const auto binding = bytes::make_span(SessionDeviceBindingMaterialValue);
	const auto hash = binding.empty()
		? openssl::Sha512(s, bytes::make_span(passcode), s)
		: openssl::Sha512(s, bytes::make_span(passcode), binding, s);
...
MTP::AuthKeyPtr CreateLegacyLocalKey(
	...
	const auto binding = SessionDeviceBindingMaterialValue;
	const auto passcodeData = binding.isEmpty()
		? passcode
		: (passcode + binding);
	PKCS5_PBKDF2_HMAC_SHA1(
		passcodeData.constData(),
		passcodeData.size(),
		(uchar*)salt.data(),
		salt.size(),
		iterationsCount,
