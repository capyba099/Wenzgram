...
void Domain::setDeviceBindingMaterial() {
	const auto path = BaseGlobalPath();
	SetSessionDeviceBindingMaterial(
		SessionDeviceBinding::bindingMaterial(path));
}

StartResult Domain::start(const QByteArray &passcode) {
	setDeviceBindingMaterial();
	...
	} else if (modern == StartModernResult::DeviceMismatch) {
		return StartResult::SessionDeviceMismatch;
...
void Domain::generateLocalKey() {
	...
	setDeviceBindingMaterial();
	_deviceBindingFingerprint = SessionDeviceBinding::storedFingerprint(
		BaseGlobalPath());
...
Domain::StartModernResult Domain::startModern(
	...
	if (!ReadFile(keyData, name, path)) {
		return StartModernResult::Empty;
	}
	LOG(("App Info: reading accounts info..."));

	QByteArray salt, deviceBinding, keyEncrypted, infoEncrypted;
	keyData.stream >> salt >> deviceBinding >> keyEncrypted >> infoEncrypted;
	...
	if (!deviceBinding.isEmpty()
		&& !SessionDeviceBinding::verifyStoredFingerprint(
			deviceBinding,
			path)) {
		LOG(("App Error: session is bound to another device or installation."));
		return StartModernResult::DeviceMismatch;
	}
	...
	_deviceBindingFingerprint = deviceBinding;
	_hasLocalPasscode = !passcode.isEmpty();
...
void Domain::writeAccounts() {
	...
	if (_deviceBindingFingerprint.isEmpty()) {
		_deviceBindingFingerprint = SessionDeviceBinding::storedFingerprint(
			path);
	}
	...
	key.writeData(_deviceBindingFingerprint);
