...
void Application::run() {
	...
	if (_sessionDeviceMismatch) {
		showSessionDeviceMismatchError();
		return;
	}
...
void Application::startDomain() {
	...
	if (state == Storage::StartResult::SessionDeviceMismatch) {
		_sessionDeviceMismatch = true;
		DEBUG_LOG(("Application Info: session device mismatch..."));
		return;
	}
...
void Application::showSessionDeviceMismatchError() {
	_lastActivePrimaryWindow->show(Ui::MakeConfirmBox({
		.text = u"Эта сессия привязана к другому устройству или установке клиента. "
			"Скопированные файлы сессии (tdata) нельзя использовать на другом компьютере.\n\n"
			"This session is bound to another device or client installation. "
			"Copied session files (tdata) cannot be used on another computer."_q,
		.confirmed = [] { Quit(); },
		.confirmText = u"OK"_q,
	}));
}
