...
void PasscodeLockWidget::submit() {
	...
	auto correct = false;
	if (domain.started()) {
		correct = domain.local().checkPasscode(passcode);
	} else {
		const auto result = domain.start(passcode);
		if (result == Storage::StartResult::SessionDeviceMismatch) {
			_error = u"Сессия привязана к другому устройству или установке клиента."_q;
			_passcode->selectAll();
			_passcode->showError();
			update();
			return;
		}
		correct = (result == Storage::StartResult::Success);
	}
	if (!correct) {
